...
# Script name: write_id_input.sh

# Prompt the user for input
read -p "Enter ORG Name (example: test01): " ORG
#read -p "Enter ORG ID: " ORG_ID
read -p "Enter Service App Github (example: https://github.com/Zero2AI/snetdevservice.git): " GITHUB
read -p "Enter Service App Github Branch (example: ocr2txt): " PROTO_BRANCH

# Write to id_input.txt with quotes
cat <<EOF > id_input.txt
ORG="Zero2ai-$ORG"
ORG_ID="Zero2ai-$ORG-ID"
GITHUB="$GITHUB"
...
if [[ -z "$ORG" || -z "$ORG_ID" || -z "$GITHUB" || -z "$PROTO_BRANCH" ]]; then
...
snet identity create --private-key 874ceb05148d7064130d9b38be1adb14103a48774bfce14d9b32ef28e377b64e FIRST key --network sepolia 2>/dev/null
...
snet organization add-group default_group \
  0xF71D6b5F99078D6ed50Ae172Fe9e73B6ef66B552 \
  http://127.0.0.1:2379 \
  2>/dev/null
...
git clone --branch $PROTO_BRANCH $GITHUB service
